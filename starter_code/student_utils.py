...
def create_tf_numeric_feature(col, MEAN, STD, default_value=0):
    ...
    '''
    tf_numeric_feature = tf.feature_column.numeric_column(key=col, default_value=default_value, normalizer_fn=functools.partial(normalize_numeric_with_zscore, mean=MEAN, std=STD), dtype=tf.float64)
...
def get_student_binary_prediction(df, col):
    ...
    '''
    df['score'] = df[col].apply(lambda x: int(x >= 5))
    df['label_value'] = df['actual_value'].apply(lambda x: int(x >= 5))
    student_binary_prediction = df[['score', 'label_value']]
    return student_binary_prediction

def get_binary_prediction(df, col):
    return df[col].apply(lambda x: int(x >= 5)).values